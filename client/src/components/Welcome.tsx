...
const ResetPassword = () => {
	...
	const resetPassword = useMutation({
		mutationFn: async (email: string): Promise<string> => {
			return await original.get(`mail/reset-password?email=${email}`).text();
		},
		onSuccess() {
			setToast({ type: 'success', text: 'Пароль отправлен на электронную почту' });
			setActivePopUp(false);
		},
		onError() {
			setToast({ type: 'error', text: 'Эл. почта не найдена' });
		}
	});
	...
	return (
		<>
			<p
				className='mt-2 cursor-pointer text-center !text-gray-400 hover:!text-c-black dark:hover:!text-c-white-2'
			...
			<PopUp isActive={activePopUp} onClose={() => setActivePopUp(false)}>
				<form className='flex w-full flex-col gap-2' onSubmit={handleSubmit(onSubmit)}>
					<p>
						Введите адрес электронной почты и мы вышлем вам новый пароль. Изменить пароль можно в настройках приложения
					</p>
					<div className='flex gap-3 sm:flex-col'>
						<input
							className={`app-input flex w-full ${errors.email && '!bg-c-pink'}`}
...
const Form = (props: FormPropsType) => {
	...
	const setToast = useToastStore((state) => state.setToast);
	const { setUser, setIsAuthorized, setIsLoading } = useUserStore();
	...
	const getUser = useMutation({
		mutationFn: async (data: CredentialType): Promise<UserType | undefined> => {
			try {
				...
				setToast({ type: 'error', text: parseErrorMessage(errorJson.message) });
			}
		},
		onSuccess(data) {
			if (data) {
				...
				setIsLoading(false);
				isRegistration ? navigate('/help') : navigate('/dashboard');
			}
		}
	});

	const onSubmit: SubmitHandler<CredentialType> = (data) => {
		getUser.mutate(data);
	};
	...
	return (
		<>
			<form
				className='flex h-[390px] flex-col justify-between gap-3 dark:bg-c-gray-5'
				onSubmit={handleSubmit(onSubmit)}>
				<div className='flex flex-col gap-3'>
					<div className='mt-5 flex flex-col gap-2'>
						<h1>{isRegistration ? 'Привет 👋' : 'С возвращением 👋'}</h1>
						<p>
							{isRegistration
								? 'Чтобы зарегистрироваться заполните поля 👇'
								: 'Заполните данные указанные при регистрации 👇'}
						</p>
					</div>
					...
					<div className='flex flex-1 flex-col gap-3'>
						{isRegistration && (
							<input
								className={`app-input ${errors.name && '!bg-c-pink'}`}
						...
						<input
							className={`app-input ${errors.email && '!bg-c-pink'}`}
							{...register('email', { required: true })}
						...
						<div className='flex flex-col gap-1'>
							<div className='relative'>
								...
								{isPassword ? (
									<EyeIcon
										className='absolute right-2 top-[14px] h-5 w-5 cursor-pointer text-c-gray-3 hover:text-black dark:hover:text-c-white-2 xs:top-[12px]'
										onClick={() => setIsPassword((state) => !state)}
									/>
								) : (
									<EyeSlashIcon
										className='absolute right-2 top-[14px] h-5 w-5 cursor-pointer text-c-gray-3 hover:text-c-black dark:hover:text-c-white-2 xs:top-[12px]'
										onClick={() => setIsPassword((state) => !state)}
									/>
								)}
							</div>
				...
				<div className='flex flex-col gap-2'>
					<Button isLoading={getUser.isLoading} disabled={getUser.isLoading}>
						Отправить
					</Button>

					<p className='text-center text-gray-400'>Отправляя форму вы соглашаетесь с политикой конфиденциальности</p>
				</div>
			</form>
			<div className='h-[20px]'>{!isRegistration && <ResetPassword />}</div>
		</>
	);
};
...
const Privacy = () => {
	...
	return (
		<>
			<p className='!text-c-gray-3 hover:!text-c-black dark:hover:!text-c-white-2' onClick={() => setActivePopUp(true)}>
			...
			<PopUp isActive={activePopUp} onClose={() => setActivePopUp(false)}>
				<div className='flex flex-col gap-2 text-sm'>
					...
					<p>
						...
						предпринимаемые Калашников Владислав Викторович (далее — Оператор).
					</p>
					<p>
					...
					<p>
						...
						ко всей информации, которую Оператор может получить о посетителях веб-сайта https://habits-tracker.ru.
					</p>
					...
					<p>
						...
						вычислительной техники. 2.2. Блокирование персональных данных — временное прекращение обработки персональных
					...
					<p>
						...
						обеспечивающих их доступность в сети интернет по сетевому адресу https://habits-tracker.ru.
					</p>
					<p>
					...
					<p>
						...
						определяемому Пользователю веб-сайта https://habits-tracker.ru.
					</p>
					<p>
					...
					<p>2.10. Пользователь — любой посетитель веб-сайта https://habits-tracker.ru.</p>
					<p>
					...
					<p>
						3.1. Оператор имеет право: — получать от субъекта персональных данных достоверные информацию и/или
						документы, содержащие персональные данные; — в случае отзыва субъектом персональных данных согласия на
						обработку персональных данных, а также, направления обращения с требованием о прекращении обработки
						персональных данных, Оператор вправе продолжить обработку персональных данных без согласия субъекта
						персональных данных при наличии оснований, указанных в Законе о персональных данных; — самостоятельно
						определять состав и перечень мер, необходимых и достаточных для обеспечения выполнения обязанностей,
					...
					<p>
						3.2. Оператор обязан: — предоставлять субъекту персональных данных по его просьбе информацию, касающуюся
						обработки его персональных данных; — организовывать обработку персональных данных в порядке, установленном
						действующим законодательством РФ; — отвечать на обращения и запросы субъектов персональных данных и их
						законных представителей в соответствии с требованиями Закона о персональных данных; — сообщать в
						уполномоченный орган по защите прав субъектов персональных данных по запросу этого органа необходимую
						информацию в течение 10 дней с даты получения такого запроса; — публиковать или иным образом обеспечивать
						неограниченный доступ к настоящей Политике в отношении обработки персональных данных; — принимать правовые,
						организационные и технические меры для защиты персональных данных от неправомерного или случайного доступа к
						ним, уничтожения, изменения, блокирования, копирования, предоставления, распространения персональных данных,
						а также от иных неправомерных действий в отношении персональных данных; — прекратить передачу
						(распространение, предоставление, доступ) персональных данных, прекратить обработку и уничтожить
						персональные данные в порядке и случаях, предусмотренных Законом о персональных данных; — исполнять иные
					...
					<p>
						4.1. Субъекты персональных данных имеют право: — получать информацию, касающуюся обработки его персональных
						...
						персональных данных; — требовать от оператора уточнения его персональных данных, их блокирования или
						...
						законом меры по защите своих прав; — выдвигать условие предварительного согласия при обработке персональных
						данных в целях продвижения на рынке товаров, работ и услуг; — на отзыв согласия на обработку персональных
						данных, а также, на направление требования о прекращении обработки персональных данных; — обжаловать в
						уполномоченный орган по защите прав субъектов персональных данных или в судебном порядке неправомерные
						действия или бездействие Оператора при обработке его персональных данных; — на осуществление иных прав,
						предусмотренных законодательством РФ.
					</p>
					<p>
						4.2. Субъекты персональных данных обязаны: — предоставлять Оператору достоверные данные о себе; — сообщать
					...
					<p>
						...
						электронных писем Персональные данные фамилия, имя, отчество электронный адрес Правовые основания
					...
					<p>
						8. Порядок сбора, хранения, передачи и других видов обработки персональных данных Безопасность персональных
					...
					<p>
						...
						kvv.prof@gmail.com с пометкой «Актуализация персональных данных».
					</p>
					<p>
						...
						Оператору уведомление посредством электронной почты на электронный адрес Оператора kvv.prof@gmail.com с
					...
					<p>
						...
						персональных данных, обратившись к Оператору с помощью электронной почты kvv.prof@gmail.com.
					</p>
					<p>
	...
	return (
		<>
			<div className=' flex w-full flex-1 items-center justify-center'>
				<div className='relative my-2 flex w-full max-w-[500px] flex-col justify-center rounded-2xl bg-c-white p-5 dark:bg-c-gray-5'>
					<div className='flex w-full justify-between gap-2'>
						<Tab isActive={isRegistration} onClick={() => setIsRegistration((state) => !state)}>
			...
			<div className='flex cursor-pointer flex-col items-center justify-center'>
				<a
					className='text-c-gray-3 hover:text-c-black dark:hover:text-c-white'
					href='https://github.com/kvvprof'
					target='_blank'
